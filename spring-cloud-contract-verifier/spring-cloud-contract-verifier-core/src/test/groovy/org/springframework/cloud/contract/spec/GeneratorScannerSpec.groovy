...
import org.springframework.cloud.contract.spec.builder.SingleTestGenerator
...
class GeneratorScannerSpec extends Specification {
	...
	def "should find all .json files and generate 6 classes for them"() {
		...
		when:
			testGenerator.generateTestClasses("org.springframework.cloud.contract.spec")
		then:
	...
	def "should create class with full package"() {
		...
		when:
			testGenerator.generateTestClasses("org.springframework.cloud.contract.spec")
		then:
			1 * classGenerator.buildClass(_, 'exceptionsSpec', 'org.springframework.cloud.contract.spec') >> "spec"
			1 * classGenerator.buildClass(_, 'exceptionsSpec', 'org.springframework.cloud.contract.spec.v1') >> "spec1"
			1 * classGenerator.buildClass(_, 'exceptionsSpec', 'org.springframework.cloud.contract.spec.v2') >> "spec2"
	}
